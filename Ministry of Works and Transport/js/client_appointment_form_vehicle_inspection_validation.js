function validate(){
	...
	// validate the last name data
	var lpnumber_element = document.getElementById('lpnumber');// retrieves the input object with ID lpnumber.
	var lpnumber_regex = /^[a-zA-Z0-9 ]{2,8}$/;

	// An alternative approach is:
	if(!lpnumber_regex.test(lpnumber_element.value)){
		lpnumber_element.value = "";
		lpnumber_element.focus();
		// write a message directly onto the web page using the innerHTML property
		var lpnumber_err = document.getElementById('lpnumber_err');
		lpnumber_err.innerHTML = "License plate number may only contain letters.";		
		valid = false;
	}
