function validate(){
	...
	// validate the last name data
	var type_element = document.getElementById('type');// retrieves the input object with ID type.
	var type_regex = /^[a-zA-Z]{2,5}$/;

	// An alternative approach is:
	if(!type_regex.test(type_element.value)){
		type_element.value = "";
		type_element.focus();
		// write a message directly onto the web page using the innerHTML property
		var type_err = document.getElementById('type_err');
		type_err.innerHTML = "Type may only contain letters.";		
		valid = false;
	}
